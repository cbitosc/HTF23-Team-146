...
def index(request):
    ...
    return render(request, 'webapp/index.html', args)
...
def restuarent(request):
    ...
    if query:
        r_object = Restaurant.objects.filter(
            Q(rname__icontains=query)).distinct()
    ...
    if request.POST:
        ...
        if type == "Modify":
            ...
            menu = Menu.objects.filter(id=menuid).\
                update(price=int(request.POST['price']), quantity=int(
...
def orderlist(request):
    ...
    orders = Order.objects.filter(
        r_id=request.user.restaurant.id).order_by('-timestamp')
    corders = []
...
def fruits_def(request):
    r_object = Restaurant.objects.all()
    return render(request, 'webapp/fruits_def.html', {'r_object': r_object})
def rest(request):
    return render(request, 'webapp/rest.html', {})